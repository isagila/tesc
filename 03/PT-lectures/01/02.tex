...
\begin{example}
  ...
  \textbf{Решение:} пусть событие \(D\)~--- \quote{выпала дама}, а событие
  \(S\)~--- \quote{выпала пика}. Обозначив \(A\) искомое событие, имеем
...
\begin{remark}
  В дальнейшем под \quote{независимыми событиями} понимаем независимые в
...
\begin{example}
  ...
  4\)) выпала шестерка. Обозначим \(B\) событие \quote{выпала хотя бы одна
  шестерка}, тогда \(B = A_1 + A_2 + A_3 + A_4\). Получаем, что \(\bar{B}\) это
  событие \quote{не выпала ни одна шестерка}, тогда \(\bar{B} = \bar{A_1} \cdot
